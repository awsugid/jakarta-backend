...
pub fn register_routes(router: Router<'_, ()>) -> Router<'_, ()> {
    router
        ...
        .get_async("/api/forms", |req, ctx| async move {
            let config =
                AppConfig::from_env(&ctx.env).map_err(|e| AppError::Internal(e.to_string()))?;
            let db = ctx.d1("DB").map_err(|e| AppError::Internal(e.to_string()))?;
        ...
        // ---------------------------------------------------------------
        .get_async("/api/forms/:kind", |_req, ctx| async move {
            let config =
                AppConfig::from_env(&ctx.env).map_err(|e| AppError::Internal(e.to_string()))?;
            let db = ctx.d1("DB").map_err(|e| AppError::Internal(e.to_string()))?;
            let repo = FormRepository::new(db);
        ...
        // ---------------------------------------------------------------
        .get_async("/api/forms/:kind/:slug", |_req, ctx| async move {
            let config =
                AppConfig::from_env(&ctx.env).map_err(|e| AppError::Internal(e.to_string()))?;
            let db = ctx.d1("DB").map_err(|e| AppError::Internal(e.to_string()))?;
            let repo = FormRepository::new(db);
            ...
            let resp = json_success(&form_status)?;
            with_cors(resp, &config.allowed_origins)
        })
        ...
        .get_async("/api/applications/:kind/:slug", |req, ctx| async move {
            let config =
                AppConfig::from_env(&ctx.env).map_err(|e| AppError::Internal(e.to_string()))?;
            let user = extract_user(&req, &ctx.env)?;
        ...
        .post_async(
            ...
            |mut req, ctx| async move {
                ...
                let linkedin_url = body["linkedin_url"].as_str().ok_or_else(|| {
                    AppError::BadRequest(
                        "Missing or invalid 'linkedin_url' field".to_string(),
                    )
                })?;
                ...
                let result =
                    validate_application(&client, &form, &user, linkedin_url).await?;
                let resp = json_success(&result)?;
                with_cors(resp, &config.allowed_origins)
            },
        )
        ...
        // ---------------------------------------------------------------
        .post_async("/api/applications/:kind/:slug/link", |req, ctx| async move {
            let config =
                AppConfig::from_env(&ctx.env).map_err(|e| AppError::Internal(e.to_string()))?;
            let _user = extract_user(&req, &ctx.env)?;
            ...
            // Check that the form is open or still editable
            let is_open = policy::is_form_open(&form);
            let is_editable = policy::is_form_editable(&form);
            if !is_open && !is_editable {
